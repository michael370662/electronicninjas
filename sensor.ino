...
#define TRIG_PIN 4
#define BUZZER_PIN 7
#define DISTANCE_INCREMENT 1000 

typedef struct EchoStatus_t
{
  bool EchoLevel;
...
volatile EchoStatus echoStatus;
unsigned int buzzerDelay;
unsigned long durationStart;
bool buzzerState;
unsigned int pPulseWidth;
...
void EchoIsr()
{
  ...
  {
    echoStatus.PulseWidth = timeStart - echoStatus.PulseStart;
    echoStatus.PulseWidthValid = true;
  }
  else
  {
    echoStatus.PulseStart = timeStart;
    echoStatus.PulseWidth = 0;
  }
}
...
void setup() {
  ...
  buzzerState = LOW;
  pPulseWidth = 0;
  echoStatus.PulseWidthValid = false;
  echoStatus.PulseStart = 0;
  echoStatus.PulseWidth = 0;
...
void loop() {
  //invalidate the pulseWidth until the ISR validates it
  echoStatus.PulseWidthValid = false;
  ...
  //wait until the ISR validates the pulse width
  while(!echoStatus.PulseWidthValid);

  uchar dataOut[PAYLOAD_WIDTH] = {0};
  
  //distance in cm
  unsigned int distance = echoStatus.PulseWidth / 58;

  Serial.println(distance);
  
  //ints are 2 byte values
  *(unsigned int*)dataOut = distance;

  //clear MAX_RT if it has been set
  clearMaxRt();
  writeTxFifo((uchar*)dataOut, PAYLOAD_WIDTH);
